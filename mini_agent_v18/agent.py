...
def assemble_tool_pool():
    ...
    for client_name, client in mcp_clients.items():
        for tool_def in client.tools:
            ...
            tools.append(tool)
            handler[tool_name] = lambda *, c=client, t=tool_def[
                "name"
            ], **kw: c.call_tool(t, kw)
...
def print_tool_query(payload: dict):
    print(f"user query: {payload['query']}")
    return {"exitCode": 0, "message": "user query printed"}


def print_stop_reason(payload: dict):
    print(f"stop reason: {payload['stop_reason']}")
    return {"exitCode": 0, "message": "stop reason printed"}


HOOKS = {
    "UserPromptSubmit": [print_tool_query],
    "SessionStart": [print_tool_name],
    "PreToolUse": [print_tool_name],
    "PostToolUse": [print_tool_output],
    "Stop": [print_stop_reason],
}
...
def tool_handler(block: dict, state: dict, tool_pool: dict) -> str:
    handler = tool_pool.get(block.name)
    try:
...
def agent_loop(messages: list, perms: PermissionManager):
    ...
    while True:
        ...
        while not queue.empty():
            try:
                # 尝试获取元素，block=False 表示队列空时立即抛异常而不阻塞
                # 这样可以避免程序无限等待
        ...
        try:
            ...
            if response.stop_reason != "tool_use":
                ...
                if not has_tool_use:
                    stop_hook_result = run_hooks(
                        "Stop", {"stop_reason": response.stop_reason}
                    )
                    if stop_hook_result["exitCode"] != 0:
                        continue
                    else:
                        return
                    ...
                    if block.name == "compact":
                        ...
                        if decision["behavior"] == "deny":
                            ...
                            if perms.ask_user(block.name, block.input or {}):
                                output = tool_handler(
                                    block, state, tool_bundle["handler"]
                                )
                            else:
                                output = f"Permission denied by user for {block.name}"
                                print(f"  [USER DENIED] {block.name}")
                        else:
                            output = tool_handler(block, state, tool_bundle["handler"])
                        results.append(
                            {
                                "type": "tool_result",
                                "tool_use_id": block.id,
            ...
            if manual_compact:
                # 如果一个工具满足下面任一条，就可以考虑不走普通 PostToolUse：
                # 它主要修改的是 messages / 上下文，而不是返回结果
                # 它改变的是循环控制流
                # 它的“结果”不适合作为普通 tool_result 消费
                # 它属于 meta-tool，而不是业务工具
                print("[manual compact]")
                messages[:] = compact.auto_compact(messages)
                continue
        except (ConnectionError, TimeoutError, OSError, APIError) as e:
            stop_hook_result = run_hooks(
                "Stop", {"stop_reason": str(e).strip().lower()}
            )
            if stop_hook_result["exitCode"] != 0:
                continue
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if query.strip().lower() in ["exit", "quit", "q"]:
            print("\nGoodbye!")
            break
        query_hook_result = run_hooks("UserPromptSubmit", {"query": query})
        if query_hook_result["exitCode"] != 0:
